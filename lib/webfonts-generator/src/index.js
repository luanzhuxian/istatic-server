...
var webfont = function(options, done) {
	...
	_.each(options.names, function(name, i) {
		if (!options.codepoints[name]) {
			// options.codepoints[name] = getNextCodepoint()
			// 每个名字对应一个unicode, 因此unicode要提前生成
			options.codepoints[name] = options.initCodePoints[i]
		}
	})
	...
	generateFonts(options)
		.then(function(result) {
			...
			result.codepoints = options.codepoints
			done(null, result)
		})
